...
def rate_rms ( rateA, rateB):
    rdiff = rateA - rateB
    rms_arr = np.zeros(rdiff.shape[0])

    for idx in np.arange(rdiff.shape[0]):
        sclip = stats.sigmaclip(rdiff[idx])
        rms_arr[idx] = np.sqrt( np.sum(sclip.clipped**2) /  (2.*float(sclip.clipped.size)) )
    return rms_arr